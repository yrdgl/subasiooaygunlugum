// src/pages/AyTakvimi.js - TAM DÜZENLENMİŞ HAMBURGER MENÜLÜ VERSİYON
import React, { useEffect, useMemo, useState } from "react";
import { useNavigate } from "react-router-dom";
import { FaMoon, FaArrowLeft, FaChevronLeft, FaChevronRight, FaBars, FaTimes, FaHome, FaBook, FaCalendarDay, FaPlus } from "react-icons/fa";
...
function AyTakvimi() {
  ...
  const [gunlukMap, setGunlukMap] = useState({});
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-white">
          ...
          <div className="flex flex-col h-full">
            <div className="p-6 border-b border-gray-800">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-white">Ay Takvimi</h2>
                  <p className="text-gray-400 text-sm">{monthTitleTR}</p>
                </div>
              </div>
            </div>
            <div className="flex-1 p-4 space-y-2">
              <button onClick={() => { setMobileMenuOpen(false); navigate("/OgrenciDashboard"); }} className="flex items-center space-x-3 p-4 bg-gray-800/50 rounded-xl border border-gray-700 hover:bg-gray-700/70">
              ...
              <button onClick={() => { setMobileMenuOpen(false); navigate("/YeniGunluk"); }} className="flex items-center space-x-3 p-4 bg-blue-900/30 rounded-xl border border-blue-700/50 hover:bg-blue-800/40">
              ...
              <button onClick={() => { setMobileMenuOpen(false); navigate("/Gunlukler"); }} className="flex items-center space-x-3 p-4 bg-purple-900/30 rounded-xl border border-purple-700/50 hover:bg-purple-800/40">
                <FaBook className="text-purple-400 text-lg" />
                <div className="flex-1">
                  <div className="text-white font-semibold">Tüm Günlükler</div>
            ...
            <div className="hidden lg:flex items-center gap-3">
              <button onClick={goPrev} disabled={!canGoPrev}
                className={`flex items-center px-3 py-2 rounded-lg border transition-colors ${
                  canGoPrev ? "border-gray-700 hover:bg-gray-800 text-gray-200" : "border-gray-800 text-gray-600 cursor-not-allowed"
                }`} title="Önceki ay">
                <FaChevronLeft className="mr-1" /> Önceki
              </button>
              <button onClick={goNext} disabled={!canGoNext}
                className={`flex items-center px-3 py-2 rounded-lg border transition-colors ${
                  canGoNext ? "border-gray-700 hover:bg-gray-800 text-gray-200" : "border-gray-800 text-gray-600 cursor-not-allowed"
                }`} title="Sonraki ay">
                Sonraki <FaChevronRight className="ml-1" />
              </button>
              <button onClick={() => navigate("/OgrenciDashboard")}
      ...
      {/* ✅ MOBİL NAV İÇİN GEZİNME BUTONLARI */}
      <div className="lg:hidden container mx-auto px-4 py-4">
        <div className="flex justify-between items-center mb-4">
          <button onClick={goPrev} disabled={!canGoPrev}
            ...
            <FaChevronLeft /> <span className="ml-1">Önceki</span>
          </button>
          <button onClick={goNext} disabled={!canGoNext}
            className={`flex items-center px-3 py-2 rounded-lg border transition-colors ${
              canGoNext ? "border-gray-700 hover:bg-gray-800 text-gray-200" : "border-gray-800 text-gray-600 cursor-not-allowed"
            }`}>
            <span className="mr-1">Sonraki</span> <FaChevronRight />
          </button>
        </div>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              ...
              <div className="text-sm text-gray-300">
                <span className="inline-flex items-center mr-4">
                  <span className="w-3 h-3 rounded-full bg-green-400 inline-block mr-2" /> Günlük var
                </span>
                <span className="inline-flex items-center">
                  <span className="w-3 h-3 rounded-full bg-gray-500 inline-block mr-2" /> Günlük yok
                </span>
              </div>
            </div>

            {loading ? (
              <div className="text-gray-300 text-center py-8">⏳ Yükleniyor...</div>
            ) : (
              <>
                ...
                <div className="grid grid-cols-7 gap-2 lg:gap-3">
                  {Array.from({ length: leadingBlanks }).map((_, i) => (
                    <div key={`blank-${monthKey}-${i}`} className="rounded-xl border border-transparent" />
                  ))}
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                      <button key={dateISO} onClick={() => handleDayClick(dateISO)}
                        ...
                        title={hasJournal ? "Bu günde günlük var (tıkla: aç)" : "Bu güne günlük ekle (tıkla: yeni günlük)"}>
                        <div className="text-lg font-bold">{day}</div>
                        {hasJournal ? (
                          <div className="mt-1 text-[11px] text-green-300 font-semibold flex items-center justify-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-green-400 inline-block" /> Aç
                          </div>
                        ) : (
                          <div className="mt-1 text-[11px] text-gray-400 font-semibold flex items-center justify-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-gray-500 inline-block" /> Yaz
                          </div>
                        )}
                      </button>
                    );
