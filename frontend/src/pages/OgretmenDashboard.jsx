...
// DEMO ÖĞRENCİ VERİLERİ - DURUMLAR GÜNCELLENDİ
const demoOgrenciler = [
  { 
    id: 1, 
    ad: "Ahmet Yılmaz", 
    sinif: "5-A", 
    egitimYili: "2026-2027", // MEVCUT YIL
  ...
  { 
    ...
    ad: "Mehmet Kaya", 
    sinif: "5-A", 
    egitimYili: "2025-2026", // GEÇEN YIL
    sonGunluk: "2025-06-15", // OKUL BİTİŞ
    durum: "Mezun", // ✅ MEZUN (Temmuz 2026'da)
    ogrenciNo: "2001",
    gunlukSayisi: 7,
    avatar: "👦"
  },
...
const demoGunlukler = [
  {
    id: 1,
    ogrenciId: 1, // Ahmet Yılmaz (Aktif)
...
function OgretmenDashboard() {
  ...
  const [seciliEgitimYili, setSeciliEgitimYili] = useState('2026-2027'); // VARSYILAN MEVCUT YIL
  const [seciliOgrenci, setSeciliOgrenci] = useState(null);

  // FİLTRE SEÇENEKLERİ
  const egitimYillari = ['Tümü', '2026-2027', '2025-2026'];
  const siniflar = ['Tümü', '5-A', '5-B'];

  // MEVCUT EĞİTİM YILI - OTOMATİK HESAPLA
  const getCurrentEgitimYili = () => {
    const now = new Date();
    const year = now.getFullYear();
    const month = now.getMonth(); // 0=Ocak, 5=Haziran
    
    // Eylül-Ağustos arası eğitim yılı
    if (month >= 8) { // Eylül'den sonra
      return `${year}-${year + 1}`;
    } else { // Ağustos'a kadar
      return `${year - 1}-${year}`;
    }
  };

  const currentEgitimYili = getCurrentEgitimYili(); // "2026-2027"
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-green-900/40 to-emerald-900/40 backdrop-blur-xl rounded-2xl border border-green-700/30 p-6">
            ...
            <p className="text-sm text-green-300 mt-2">{currentEgitimYili} öğrencisi</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-purple-900/40 to-pink-900/40 backdrop-blur-xl rounded-2xl border border-purple-700/30 p-6">
            ...
            <p className="text-sm text-purple-300 mt-2">2025-2026 ve öncesi</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-2">
                <span className="text-green-400">✅ {currentEgitimYili}:</span> Aktif öğrenciler<br/>
                <span className="text-yellow-400">🎓 2025-2026:</span> Mezun öğrenciler (Temmuz 2026'da mezun oldu)
              </p>
            </div>
          ...
          <div className="p-8 border-b border-white/10">
            <div className="flex justify-between items-center">
              ...
              <div className="text-sm bg-gray-900/50 px-4 py-2 rounded-xl border border-gray-700">
                <span className="text-blue-400">ℹ️</span> Durumlar: 
                <span className="text-green-400 mx-2">Aktif</span> | 
                    ...
                    <tr key={ogrenci.id} className="hover:bg-white/5 transition-colors">
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-400">
                              No: {ogrenci.ogrenciNo} • Son: {ogrenci.sonGunluk}
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-6 text-gray-300">
                        <div className="flex items-center gap-2">
                          <span>📅</span>
                          <span className={ogrenci.egitimYili === currentEgitimYili ? "text-green-400" : "text-yellow-400"}>
                            {ogrenci.egitimYili}
                          </span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <span className={`px-4 py-2 rounded-xl text-sm font-medium ${
                          ...
                          {ogrenci.durum} {ogrenci.egitimYili === '2025-2026' && '(Temmuz 2026)'}
                        </span>
                      </td>
                      <td className="px-8 py-6">
                      ...
                      <td className="px-8 py-6">
                        <button
                          ...
                          className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-xl font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <div className="p-6 bg-gradient-to-r from-blue-900/20 to-purple-900/20 text-center text-gray-300 text-sm border-t border-white/10">
            <div className="flex flex-col items-center gap-2">
              <div className="flex items-center gap-4">
                <span className="text-green-400">✅ {currentEgitimYili}:</span>
                <span>Aktif öğrenciler (şu an okulda)</span>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-yellow-400">🎓 2025-2026:</span>
                <span>Mezun öğrenciler (Temmuz 2026'da mezun oldu)</span>
              </div>
            </div>
          </div>
        </div>
