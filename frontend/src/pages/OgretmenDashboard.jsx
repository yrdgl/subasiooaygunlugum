// src/pages/OgretmenDashboard.js - TAM DÜZENLENMİŞ HAMBURGER MENÜLÜ VERSİYON
import React, { useEffect, useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  FaHome, FaSignOutAlt, FaBars, FaTimes, FaMoon,
  FaSearch, FaFilter, FaArrowLeft, FaUserCircle, FaBook,
  FaCalendarAlt, FaStar, FaChartBar, FaUsers
} from 'react-icons/fa';
import {
  collection,
  getDocs,
...
function OgretmenDashboard() {
    ...
    const loadStudents = async () => {
      try {
        ...
        const list = snap.docs.map((d) => {
          ...
          const sinifGosterim = (sinif && sube) ? `${sinif}-${sube}` : (sinif ? `${sinif}` : (data.sinifGosterim || "-"));
        ...
        showToast("error", "Yükleme Hatası", "Öğrenci listesi yüklenemedi.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleYildizVer = async (gunlukId, yeniYildiz) => {
    ...
    try {
      ...
      if (eskiYildiz > 0 && yildiz < eskiYildiz) {
        showToast("error", "Yıldız Azaltılamaz ⚠️", `Bu günlük zaten ${eskiYildiz} yıldız aldı. Yıldızı sadece artırabilirsiniz.`);
        return;
      }
      ...
      setGunlukler(prev => prev.map(g => (
        g.id === gunlukId
          ? { ...g, ogretmenYildizi: yildiz, ogretmenYildizVerildi: yildiz > 0, yildiz: yildiz }
          : g
      )));

      if (yildiz > 0) {
        if (eskiYildiz === 0) {
          showToast("success", "Yıldız Verildi & Kilitlendi ✅", `⭐ ${yildiz} yıldız verildi! Günlük kilitlendi.`);
        } else if (yildiz > eskiYildiz) {
          showToast("success", "Yıldız Artırıldı 📈", `Yıldız ${eskiYildiz} → ${yildiz} olarak güncellendi.`);
        } else {
      ...
      showToast("error", "Kayıt Hatası ❌", "Yıldız kaydedilemedi.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0e27] via-[#1a1f3a] to-[#0a0e27] text-white">
          ...
          <div className="flex flex-col h-full">
            <div className="p-6 border-b border-gray-800">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-white">Öğretmen Paneli</h2>
                  <p className="text-gray-400 text-sm">5. Sınıf Öğretmeni</p>
                </div>
              </div>
            </div>
            <div className="flex-1 p-4 space-y-2">
              ...
              <button onClick={() => { setMobileMenuOpen(false); handleLogout(); }} className="flex items-center space-x-3 w-full p-4 bg-red-900/30 rounded-xl border border-red-700/50 hover:bg-red-800/40">
                <FaSignOutAlt className="text-red-400 text-lg" />
                <div className="flex-1 text-left">
                  <div className="text-white font-semibold">Çıkış Yap</div>
                  <div className="text-gray-400 text-sm">Öğretmen panelinden çık</div>
                </div>
              </button>
            </div>
          ...
          <div className={`bg-gray-900/90 backdrop-blur-md border ${t.ring} rounded-2xl shadow-xl px-4 sm:px-5 py-3 sm:py-4`}>
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <div className="flex-1">
                ...
                {toast.message && <div className={`text-xs sm:text-sm mt-1 ${t.msg}`}>{toast.message}</div>}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="fixed inset-0 bg-cover bg-center opacity-20" style={{ backgroundImage: 'url(https://customer-assets.emergentagent.com/job_moontracker-5/artifacts/zksvk4wp_AY%20ARKAPLAN.jpg)' }} />
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-3 sm:p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
          <div className="bg-gradient-to-br from-blue-900/40 to-indigo-900/40 backdrop-blur-xl rounded-2xl border border-blue-700/30 p-4 sm:p-6">
            ...
            <p className="text-xs sm:text-sm text-blue-300 mt-1 sm:mt-2">{seciliEgitimYili === 'Tümü' ? 'Tüm yıllar' : seciliEgitimYili}</p>
          </div>
        ...
        <div className="bg-gradient-to-br from-[#1a1f3a]/80 to-[#0a0e27]/80 backdrop-blur-xl rounded-2xl border border-white/10 p-4 sm:p-6 mb-6 sm:mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
            <div>
              <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 text-gray-300 flex items-center gap-2"><FaCalendarAlt /> Eğitim Yılı</h3>
              <div className="flex flex-wrap gap-2">
                {egitimYillari.map(yil => (
                  <button key={yil} onClick={() => setSeciliEgitimYili(yil)}
                    className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg transition-all text-xs sm:text-sm ${
                      seciliEgitimYili === yil
                        ? yil === currentEgitimYili ? 'bg-gradient-to-r from-green-600 to-emerald-600 text-white shadow-md'
                        : yil === '2026-2027' ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-md'
            ...
            <div>
              <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 text-gray-300 flex items-center gap-2"><FaFilter /> Sınıf</h3>
              <div className="flex flex-wrap gap-2">
                {siniflar.map(sinif => (
                  <button key={sinif} onClick={() => setSeciliSinif(sinif)}
                    className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg transition-all text-xs sm:text-sm ${
                      seciliSinif === sinif
              ...
              <p className="text-gray-400 text-xs sm:text-sm mt-2">Sadece 5. sınıf öğrencileri (5-A ve 5-B)</p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-gradient-to-br from-[#1a1f3a]/80 to-[#0a0e27]/80 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden mb-6 sm:mb-8">
            <div className="p-4 sm:p-6 md:p-8 border-b border-white/10 bg-gradient-to-r from-blue-900/30 to-purple-900/30">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3 sm:gap-4">
                  <button onClick={handleGeriDon} className="text-gray-400 hover:text-white transition-colors text-base sm:text-lg">
                    <FaArrowLeft className="inline mr-2" /> Geri
                  </button>
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="text-3xl sm:text-4xl md:text-5xl">{seciliOgrenci.avatar || "👤"}</div>
                    <div>
                      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-1">{seciliOgrenci.ad}</h2>
                      <p className="text-gray-300 text-xs sm:text-sm">{seciliOgrenci.sinif} • {seciliOgrenci.egitimYili} • No: {seciliOgrenci.ogrenciNo}</p>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-4 sm:p-6 md:p-8">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 gap-3">
                <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white flex items-center gap-2 sm:gap-3"><FaBook /> Günlük Kayıtları ({ogrenciGunlukleri.length})</h3>
                <div className="text-xs sm:text-sm bg-yellow-900/30 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl border border-yellow-700/50 w-fit">
                  <FaStar className="inline mr-1 text-yellow-300" /> Öğrenciler yıldızlarını görecek
                </div>
              </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 sm:mb-4 gap-3">
                        <div className="flex-1">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-gray-300 text-xs sm:text-sm">
                            <span className="flex items-center gap-1"><FaCalendarAlt /> {gunluk.tarih}</span>
                            <span className="flex items-center gap-1">{gunluk.ayFazi}</span>
                      ...
                      <div className="bg-gray-900/30 rounded-xl p-3 sm:p-4 mt-3 sm:mt-4 border border-gray-800/50">
                        ...
                        <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-800/50">
                          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
                            <div className="flex-1">
                              <h5 className="font-bold text-white mb-2 flex items-center gap-2 text-sm sm:text-base"><FaStar className="text-yellow-400" /> Öğretmen Değerlendirmesi</h5>
                            ...
                            <div className="lg:text-right">
                              ...
                              <div className="flex flex-wrap gap-1 justify-start lg:justify-end">
                                {[1, 2, 3, 4, 5].map(yildiz => (
                                  <button key={yildiz} onClick={() => handleYildizVer(gunluk.id, yildiz)}
                                    ...
                                    {gunluk.ogretmenYildizi > 0 && yildiz < gunluk.ogretmenYildizi && <span className="hidden sm:inline"> ⬇️</span>}
                                    {gunluk.ogretmenYildizi > 0 && yildiz > gunluk.ogretmenYildizi && <span className="hidden sm:inline"> ⬆️</span>}
                                  </button>
                                ))}
                              </div>
          ...
          <div className="bg-gradient-to-br from-[#1a1f3a]/80 to-[#0a0e27]/80 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
            <div className="p-4 sm:p-6 md:p-8 border-b border-white/10">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                  <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-1 sm:mb-2"><FaUsers /> 5. Sınıf Öğrenci Listesi</h2>
                ...
                <div className="text-xs sm:text-sm bg-gray-900/50 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl border border-gray-700 w-fit">
                  <span className="text-green-400">✅ Aktif</span> • <span className="text-purple-400 mx-1 sm:mx-2">🔮 Gelecek</span>
                </div>
              </div>
            </div>
                    ...
                    <div key={ogrenci.id} className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-xl rounded-2xl border border-white/10 p-4">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-400">No: {ogrenci.ogrenciNo} • {ogrenci.sinif}</div>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        <div className="text-center">
                          <div className="text-xs text-gray-400 mb-1">Eğitim Yılı</div>
                          <div className={`text-sm font-medium ${ogrenci.egitimYili === currentEgitimYili ? "text-green-400" : "text-purple-400"}`}>
                            {ogrenci.egitimYili}
                          </div>
                        </div>
                        <div className="text-center">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <button onClick={() => handleOgrenciSec(ogrenci)}
                        ...
                        <tr key={ogrenci.id} className="hover:bg-white/5 transition-colors">
                          <td className="px-4 sm:px-6 md:px-8 py-4">
                            <div className="flex items-center">
                              ...
                              <div>
                                ...
                                <div className="text-xs sm:text-sm text-gray-400">No: {ogrenci.ogrenciNo}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 sm:px-6 md:px-8 py-4">
                            <span className="px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-blue-900/50 to-blue-800/50 text-blue-300 rounded-xl text-xs sm:text-sm font-medium border border-blue-700/50">{ogrenci.sinif}</span>
                          </td>
                          <td className="px-4 sm:px-6 md:px-8 py-4">
                            <div className="flex items-center gap-2">
                              <FaCalendarAlt />
                              <span className={ogrenci.egitimYili === currentEgitimYili ? "text-green-400" : "text-purple-400"}>
                                {ogrenci.egitimYili}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="px-4 sm:px-6 md:px-8 py-4">
                            <div className="flex items-center gap-2 sm:gap-3">
                              ...
                              <span className={`font-bold text-base sm:text-lg md:text-xl ${(ogrenci.gunlukSayisi || 0) > 0 ? 'text-white' : 'text-gray-500'}`}>
